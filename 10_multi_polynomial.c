/* program to multiply two polynomial.
...
poly *mul_poly(poly *p1, poly *p2, int size_of_1, int size_of_2, int *size_of_3)
{

    poly *p3 = (poly *)malloc(size_of_1 * size_of_2 * sizeof(poly));
    for (int i = 0; i < size_of_1 * size_of_2; i++)
    {
        POW(p3, i) = COF(p3, i) = 0;
    }
    int size3 = 0;
    ...
    while (j < size_of_1)
    {
        ...
        int i = 0;
        poly *p4 = (poly *)malloc(size_of_2 * sizeof(poly));
        while (i < size_of_2)
        {
            POW(p4, i) = POW(p1, j) + POW(p2, i);
            COF(p4, i) = COF(p1, j) * COF(p2, i);
            i++;
        }

        p3 = add_poly(p3, p4, size3, size_of_2, &size3);
        p4 = 0;
        j++;
    }
...
int main()
{
    ...
    p1 = read_poly(size1);
    p2 = read_poly(size2);
    p3 = mul_poly(p1, p2, size1, size2, &size3);
    printf("p1\n");
